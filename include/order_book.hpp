...
//however beyond just those deleting orders by order id is needed so an sorted map will be used
...
public:
    ...
    OrderBook();
    std::vector<Fill> limit_buy(OrderId id, Price price, Quantity quantity);
    std::vector<Fill> limit_sell(OrderId id, Price price, Quantity quantity);
